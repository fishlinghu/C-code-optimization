...
runTest( int argc, char** argv) 
    ...
    int *input_itemsets, *referrence, *rand_seq_1, *rand_seq_2;
	int size;
    ...
    rand_seq_1 = (int *)malloc( max_cols * sizeof(int) );
    rand_seq_2 = (int *)malloc( max_cols * sizeof(int) );
	...
	for(i=1; i< max_rows ; i++)
        ...
        rand_seq_1[i] = rand() % 10 + 1;
        }
    for(j=1; j< max_cols ; j++)
        {    //please define your own sequence.
        //input_itemsets[j] = rand() % 10 + 1;
        rand_seq_2[j] = rand() % 10 + 1;
        }


	/*for (i = max_cols - 1 ; i > 0; --i)
        {
        //col_of_blosum62 = input_itemsets[i * max_cols];
		col_of_blosum62 = rand_seq_1[i];
        for (j = max_rows - 1 ; j > 0; --j)
            {
            // the value of the elements of input_itemsets cannot exceed 23
            referrence[i * max_cols + j] = blosum62[ col_of_blosum62 ][ rand_seq_2[j] ];
            //cout << referrence[i * max_cols+j] << endl;
            }
        }*/
    //cout << "!!!!!!!!!!!" << endl;
    for(i = 1; i< max_rows ; i++)
    ...
    while(i < max_cols - 1)
        ...
        while(j < max_rows - 1)
            {
            index = i * max_cols + j;
            referrence[index] = blosum62[ col_of_blosum62 ][ rand_seq_2[j] ];
    ...
    //Don't remove it
    printf("***************** Time for main computation *****************\n");
    printf("s_time.tv_sec:%ld, s_time.tv_nsec:%09ld\n", mid_time.tv_sec, mid_time.tv_nsec);
    ...
    if(end_time.tv_nsec > mid_time.tv_nsec)
        ...
        end_time.tv_nsec - mid_time.tv_nsec);
        }
    else
        {
        printf("[diff_time:%ld.%09ld sec]\n",
        end_time.tv_sec - mid_time.tv_sec - 1,
        end_time.tv_nsec - mid_time.tv_nsec + 1000*1000*1000);
        }
    printf("\n***************** Total time *****************\n");
